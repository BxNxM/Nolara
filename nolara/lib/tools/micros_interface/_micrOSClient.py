...
class micrOSClient:
    ...
    def telnet_prompt(self):
        return f"{self.preprompt}{self.prompt} "
    ...
    def __receive_data(self, read_timeout=20, stream=False):
        ...
        if select.select([self.conn], [], [], read_timeout)[0]:
            while True:
                ...
                if stream:
                    incoming_data = incoming_data.replace(self.prompt, f"{self.prompt}")
    ...
    def __run_command(self, cmd, stream=False):
        ...
        print(f"[micrOSClient] prompt mismatch, hostname: {check_hostname} prompt: {check_prompt} ")
        # Check UID?
        return None
    ...
    def send_cmd(self, cmd, timeout=3, retry=5, stream=False):
        ...
        try:
            ...
            self.dbg_print(f"[ERR] send_cmd error: {e}")
        ...
        self.avg_reply[1] += 1
        f_delta_t = "[{:.2f}]".format(delta_time)
        self.dbg_print("{}[⏰] {} reply: {}".format(f_delta_t, cmd, out))
    ...
    def dbg_print(self, msg, end='\n'):
        if self.dbg:
            print(f"{' '*self.spacer}[dbg] {msg}", end=end)
            if self.spacer < 60:
                self.spacer += 1
...
def micros_connection_metrics(address):
    ...
    print(f" #### Single connection Load test (TCP)")
    for k in verdict_list:
        print(f"\t{k}")
    print(f" #### Multi connection Load test (TCP) - reconnect measurement")
